...
seq = iaa.Sequential([
    ...
    iaa.Crop(percent=(0, 0.2)), # 裁剪图像
    iaa.Multiply((0.8, 1.2)), # 改变亮度
    iaa.AdditiveGaussianNoise(scale=(1, 20)), # 添加高斯噪声
])
def load_and_augment(image_path, label_path, save_dir_images, save_dir_labels):
    # 读取图像
    ...
    # 保存增强后的图像
    image_aug_path = os.path.join(save_dir_images, os.path.basename(image_path))
    cv2.imwrite(image_aug_path, image_aug)

    # 将增强后的标注转换回YOLO格式并保存
    label_aug_path = os.path.join(save_dir_labels, os.path.basename(label_path))
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for image_file in os.listdir(imagePath):
            ...
            task = executor.submit(load_and_augment, image_path, label_path, 'augmented_images', 'augmented_labels')
            futrues.append(task)
